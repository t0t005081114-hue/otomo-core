...
- Status: **Suspended**（2026-09-25 Human Decision。`harness/REMOTE_REVIEW.md` Suspension）。Suspension時点の版: Draft v0.1（Pilot: OTOMO LAB）
- 対象: Runnerを登録・運用する人間（Owner）
- Related: `harness/REMOTE_REVIEW.md` / `harness/REMOTE_REVIEW_SECURITY.md`

Remote Review PilotはSuspended / Non-defaultである。新規setup・Runner再構築を標準運用として行わない。再開のHuman Decisionまで、本書はReference onlyである。以下の手順・Acceptance手順・検証済み事項は、過去PilotのHistorical Referenceとして保持する。
...
- 2026-09-25 Suspended（Human Decision、docs-only、Forced Level 2扱い）: Remote Review PilotのSuspension（`harness/REMOTE_REVIEW.md` Suspension）に合わせてStatusを更新し、新規setup・Runner再構築を標準運用として行わないこと、再開のHuman Decisionまで本書をReference onlyとすることを冒頭に明記した。**§0〜§9の手順・script・Acceptance手順・検証済み事項は変更していない**
