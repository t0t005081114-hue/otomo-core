...
- **Runner directory側**: ACLを明示的にhardeningする（**こちらは既定のNTFS権限のままにしない**）。access は `NT AUTHORITY\SYSTEM` / `BUILTIN\Administrators` / 専用runner user に限定し、通常のauthenticated userにwrite / modifyを残さない（下記 Runner Directory Integrity）
...
2. **書き込み側**: Runner directoryのACLが、SYSTEM / Administrators / 専用runner user に限定されている（Service modeではrunnerが作る `GITHUB_ActionsRunner_*` groupを含めてよい）
