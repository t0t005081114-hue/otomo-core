...
| SEC-26 | report jobは、`status: PASS` を自己申告するcheckについて、`command` がtrusted `config.checks` から導出したcanonical invocation（`canonicalCommand()`。`npm_script` checkは `["npm", "run", <npm_script>]`、argv checkはtrusted `command` 配列そのもの）と一致すること、`exit_code === 0`・`timed_out === false`・`skip_reason === null`・`failure === null` であることを検証する。いずれかを満たさなければEvidenceの自己申告verdictを採用せずINCOMPLETEとする。command文字列はreview job自身の実行（`planCheck`）とこの照合の両方が同一関数から導出し、別途ハードコードしない（2026-09-15追加、CORE-RR-FRR-001） | `lib/checks.mjs`（`canonicalCommand`, `validateCheckManifest`）+ `report.mjs` + unit test（AT-28） |
